...
export default function LoginPage() {
    ...
    const handleEmailLogin = async () => {
        ...
        try {
            ...
            if (authError) {
                setError(authError.message || "Login gagal. Periksa email dan password kamu.");
            } else {
    ...
    return (
        <div className="flex w-1/2 h-screen flex-col gap-4 border-r bg-surface-card border-border justify-center items-center">
            <div className="md:w-[500px] w-full flex gap-6 flex-col">
                ...
                <Button variant="white" onClick={handleEmailLogin} disabled={loading}>
                ...
                <div className="flex items-center justify-center gap-2">
                    <p className="text-white/70 font-ibm-plex-mono text-xs uppercase">Don&apos;t have an account?</p>
                    <Link href="/register" className="text-white font-ibm-plex-mono text-xs uppercase underline">Register</Link>
                </div>
            </div>
        </div>
    )
